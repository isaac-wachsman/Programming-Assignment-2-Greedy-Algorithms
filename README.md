...
Let $A_i$ denote an algorithm that matches the decisions of OPTFF for the first $i$ requests. We will show that there exists an algorithm $A_{i+1}$ that matches the decisions of OPTFF for the first $i+1$ requests and the number of misses for $A_{i+1}$ is less than or equal to the number of misses for $A_i$.
...
This shows that we can transform an arbitrary algorithm A that agrees with OPTFF on some number of requests (which can be 0 initially), into an algorithm that agrees on all requests (which means A is transformed into OPTFF) where the number of misses for OPTFF is bounded above by the number of misses for A. Since A is arbitrary, OPTFF is optimal.
